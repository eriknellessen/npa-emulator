...
import static com.npa.androsmex.tools.Converter.bigIntToByteArray;
import static com.npa.androsmex.tools.Converter.byteArrayToECPoint;
...
//				"23C97278E4CD748048AC56BA5BDC3D46",16);
	 ...
	 */
	@Override
	public byte[] getY1(byte[] s) {
		nonce_s = s;
		byte[] x1 = new byte[(curve.getFieldSize()/8)];
		randomGenerator.nextBytes(x1);
		PCD_SK_x1 = new BigInteger(1, x1);
		// by Ole
		PCD_SK_x1 = new BigInteger("19C428715663DE745D1824B855D2B967" +
				"890C99D68ED5FEEE9DCDF8D7BBA289D2",16);
	...
	// by Ole
	private ECPoint getY2(ECPoint.Fp X1) {
		PICC_PK_Y1 = X1;
		calculateSharedSecretP(); // berechnet P
		calculateNewPointG(); // berechnet G'
		byte[] x2 = new byte[(curve.getFieldSize()/8)];
		...
		// by Ole
		PCD_SK_x2 = new BigInteger("15872C56908C144002177994CFAAEDD5" +
				"467CE150853C44535051FF24183039D8",16);
	...
	// by Ole
	@Override
	public byte[] getY2(byte[] X1Bytes) {
		ECPoint.Fp Y1 = null;
		try {
			Y1 = (Fp) byteArrayToECPoint(X1Bytes, curve);
		} catch (Exception e) {
			System.err.println(e.toString());
			e.printStackTrace();
		}
		return getY2(Y1).getEncoded();
	}
